/**
 * Apply calorie_bank migration — loads .env.local, .env.vercel.prod
...
const migrationPath = resolve(root, 'supabase/migrations/20250618120000_calorie_bank.sql')
...
async function verifyWithServiceRole() {
  ...
  const res = await fetch(`${url}/rest/v1/calorie_bank?select=id&limit=1`, {
...
if (post.ok) {
  ...
  const rows = await fetch(`${url}/rest/v1/calorie_bank?select=*&limit=1`, {
    headers: { apikey: key, Authorization: `Bearer ${key}` },
  })
  console.log('SELECT_LIMIT_1', rows.status, await rows.text())
}
