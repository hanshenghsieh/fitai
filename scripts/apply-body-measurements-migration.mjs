/**
 * Apply body_measurements migration — loads .env.local, .env.vercel.prod
...
const migrationPath = resolve(root, 'supabase/migrations/20250627120000_body_measurements.sql')
...
async function verifyWithServiceRole() {
  ...
  const res = await fetch(`${url}/rest/v1/body_measurements?select=id&limit=1`, {
