#!/usr/bin/env npx tsx
/**
 * Menu Backfill Sprint 3 — merge 50-brand batch into staging manifest.
...
const BRANDS_PATH = path.join(ROOT, 'data', 'food-kb', 'staging', 'sprint-3', 'brands.json')
const MANIFEST_IN = path.join(ROOT, 'data', 'food-kb', 'staging', 'manifest.json')
const REPORT_OUT = path.join(ROOT, 'docs', 'MENU_BACKFILL_SPRINT_3_REPORT.md')

const VERIFIED_AT = new Date().toISOString()
const VERIFIED_BY = 'menu-backfill-sprint-3'

function main() {
  if (BDGS_DATA_FREEZE) {
    console.error('\nBDGS_DATA_FREEZE is active. Founder must lift freeze before Sprint 3 build.\n')
    process.exit(1)
  }
  ...
  const sprint3Names = new Set(brands.map(b => b.canonical_name))
  ...
  const preserved =
    existing?.restaurants.filter(r => !sprint3Names.has(r.canonical_name)) ?? []

  const buildOpts = {
    brands,
    verifiedAt: VERIFIED_AT,
    verifiedBy: VERIFIED_BY,
    itemIdPrefix: 'sprint3-',
    sprintLabel: 'Sprint3',
  }

  const sprint3Restaurants = buildStagingRestaurants(buildOpts)
  runStagingRestaurantQa(sprint3Restaurants, VERIFIED_AT)

  const restaurants = [...preserved, ...sprint3Restaurants]
  const conflicts = collectStagingConflicts(sprint3Restaurants)
  ...
  const stats = computeSprintReportStats(brands, sprint3Restaurants)
  const total = restaurants.reduce((n, r) => n + r.items.length, 0)

  console.log('\n=== Menu Backfill Sprint 3 ===\n')
  console.log(`Preserved: ${preserved.length}`)
  console.log(`Sprint 3 with items: ${stats.withItems}/${brands.length}`)
  console.log(`Sprint 3 new items: ${stats.totalItems}`)
