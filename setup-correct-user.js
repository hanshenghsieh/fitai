...
async function setupCorrectUser() {
  const email = 'kevinkknn84@gmail.com';
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/weekly_plans?user_id=eq.${userId}`, {
    method: 'DELETE',
    headers: {
      Authorization: `Bearer ${SERVICE_KEY}`,
      apikey: SERVICE_KEY,
  ...
  if (createRes.ok) {
    console.log('✅ Plan created for correct user!');
    console.log('🔗 Reload dashboard now');
  } else {
    console.log('❌ Failed:', await createRes.text());
  }
}

setupCorrectUser().catch(console.error);
