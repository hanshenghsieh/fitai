...
async function updateCorrectWeek() {
  ...
  const plan = {
    ...
    days: Array.from({ length: 7 }, (_, i) => ({
      day: i + 1,
      date: new Date(new Date(weekStart).getTime() + i * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      meals: [
  ...
  if (updateRes.ok) {
    console.log('✅ Plan updated with correct week!');
  } else {
    console.log('❌ Failed:', await updateRes.text());
  }
}

updateCorrectWeek().catch(console.error);
