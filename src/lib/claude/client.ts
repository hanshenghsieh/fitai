...
辨識規則（重要，依序判斷，適用任何食物，不限特定菜色）：
1. 先判斷這張照片是不是「一道已經組合完成的料理」（例如：沙拉、拌菜、便當、丼飯、湯品、炒飯炒麵等食材已經混合或搭配在一起的餐點）。如果是，優先把它當成一整道料理來命名，不要因為看到不同顏色、切片、碎塊、配料，就自動拆成多個獨立食物用途猜測。
2. 只有當照片中明顯存在「彼此獨立、分開擺放、可以各自單獨食用」的多項食物時（例如便當裡分格的主菜、配菜、白飯各自一格），才輸出多個 items，每項食物各自列出。
3. 無法確定細部配料是什麼時，不要為了填滿多個 items 硬猜測；不確定就保守處理，寧可用整道料理名稱概括，也不要編造看起來合理但沒把握的配料名稱。
4. "is_composite_dish"：這張照片是否為單一已組合完成的料理（true/false）。
5. "dish_name"：is_composite_dish 為 true 時，填整道料理的名稱；is_composite_dish 為 false（畫面中真的是多項各自獨立的食物）時可留空。
...
輸出 JSON：
{
  "is_composite_dish": true|false,
  "dish_name": "整道料理名稱（is_composite_dish 為 true 時填寫，否則可留空字串）",
  "items": [{
    "name": "食物名稱",
    "portion": "份量描述（可選）",
    "confidence": "high"|"medium"|"low"
  }],
  "meal_summary": "一句話描述整餐"
}
只輸出 JSON。`,
          },
        ],
