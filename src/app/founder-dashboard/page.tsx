...
function ProductCoreRows({ label, snapshot }: { label: string; snapshot: DashboardSnapshot }) {
  ...
  return (
    <>
      ...
      <Row label="失敗次數" value={snapshot.photo.failure} />
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 20px' }}>
      ...
      <Card title="錯誤總覽（過去 24 小時）">
        <Row label="食物拍照失敗次數（今日）" value={today.photo.failure} />
        <Row label="食物拍照失敗次數（過去 7 天）" value={last7Days.photo.failure} />
        <p style={{ fontSize: 12, color: '#888', marginTop: 8 }}>
